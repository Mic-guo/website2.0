...
// Torus dimensions
const torusRadius = 0.55      // distance from center to tube center
const tubeRadius  = 0.28      // tube thickness
const geometry = new THREE.TorusGeometry(torusRadius, tubeRadius, 32, 80)

// Physics approximation: ring of small spheres around the torus tube.
// Using 8 spheres evenly spaced around the major circle gives good collision fidelity.
function createTorusColliders(rigidBody) {
  const segments = 12
  const sphereR = tubeRadius * 0.85   // slightly smaller to avoid ghost contacts
  ...
  // Central ball collider to fill the hollow core a bit
